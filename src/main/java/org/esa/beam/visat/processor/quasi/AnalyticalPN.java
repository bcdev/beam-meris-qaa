...
public class AnalyticalPN extends ProcessingNode {
	...
	public static final String ANALYSIS_FLAG_BAND = "analytical_flag";
	...
	protected Product createTargetProductImpl() {
		...
		for (int bandIndex = 0; bandIndex < reflectanceBandNames.length; bandIndex++) {
			...
			reflectanceBands[bandIndex] = sourceProduct
					.getBand(reflectanceBandName);
			if (reflectanceBands[bandIndex] == null) {
				throw new IllegalArgumentException("Source product does not "
						+ "contain band " + reflectanceBandName + ".");
			}
		}
		...
		Product outputProduct = new Product(
				AnalyticalPN.DEFAULT_OUTPUT_PRODUCT_NAME,
		...
		for (int i = 0; i < QaaABands.length; i++) {
			QaaABands[i] = new Band(qaa_a + wavel[i], ProductData.TYPE_FLOAT32,
					sceneWidth, sceneHeight);
			QaaABands[i].setDescription("Quasi-Analytical a - " + wavel[i]
					+ "nm");
		...
		for (int i = 0; i < QaaBbBands.length; i++) {
			QaaBbBands[i] = new Band(qaa_bb + wavel[i],
					ProductData.TYPE_FLOAT32, sceneWidth, sceneHeight);
			QaaBbBands[i].setDescription("Quasi-Analytical bb - " + wavel[i]
					+ "nm");
		...
		for (int i = 0; i < QaaAphBands.length; i++) {
			QaaAphBands[i] = new Band(qaa_aph + wavel[i],
					ProductData.TYPE_FLOAT32, sceneWidth, sceneHeight);
			QaaAphBands[i].setDescription("Quasi-Analytical aph - " + wavel[i]
					+ "nm");
		...
		for (int i = 0; i < QaaAdgBands.length; i++) {
			QaaAdgBands[i] = new Band(qaa_adg + wavel[i],
					ProductData.TYPE_FLOAT32, sceneWidth, sceneHeight);
			QaaAdgBands[i].setDescription("Quasi-Analytical adg - " + wavel[i]
					+ "nm");
...
*/
	 ...
	 *            progress monitor
	 */
	protected void processFrame(int frameX, int frameY, int frameW, int frameH,
			...
			for (int i = 0; i < frameSize; i++) {
				...
				try {
					if (flagData[i] < cloud_threshold) { // Check if it is water or not
						...
						qaaf_v5(pixel, rrs_pixel, a_pixel, bbp_pixel,
								SubProgressMonitor.create(pm, 1));

						// steps 7-10
						qaaf_decomp(rrs_pixel, a_pixel, aph_pixel, adg_pixel,
								SubProgressMonitor.create(pm, 1));

						analyticalFlagData[i] = FLAG_VALID;

						for (int b = 0; b < a.length; b++) {
							a[b][i] = (float) aw[b] + aph_pixel[b]
									+ adg_pixel[b];
	...
	protected void qaaf_v5(float[] Rrs, float[] rrs, float[] a, float[] bbp,
		...
		if ( Rrs[idx670]>Rrs670_upper || Rrs[idx670]<Rrs670_lower
		...
		for (int b = 0; b < Rrs.length-1; b++) {
			...
			if (nom >= 0) {
				...
				throw new ImaginaryNumberException(
						"Will produce an imaginary number", nom);
			}
		}
		pm.worked(1);
		...
		denom = (float) (rrs[idx560] + 5 * rrs[idx670]
				* (rrs[idx670] / rrs[idx490]));
		...
		rho = (float) (acoefs[0] + acoefs[1] * rho + acoefs[2]
				* Math.pow(rho, 2.0));
		...
		// step 5
		for (int b = 0; b < Rrs.length-1; b++) {
			bbp[b] = (float) (bbp560 * Math.pow((float) wavel[idx560]
					/ (float) wavel[b], Y));
		}
		pm.worked(1);
	...
	protected void qaaf_decomp(float[] rrs, float[] a, float[] aph,
		...
		for (int b = 0; b < rrs.length-1; b++) {
			adg[b] = (float) (ag440 * Math.exp(-1 * S
					* (wavel[b] - wavel[idx440])));
