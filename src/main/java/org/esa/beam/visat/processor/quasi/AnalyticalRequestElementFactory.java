...
public class AnalyticalRequestElementFactory implements RequestElementFactory {
    ...
    private static AnalyticalRequestElementFactory instance;
    ...
    public static AnalyticalRequestElementFactory getInstance() {
        if (instance == null) {
            instance = new AnalyticalRequestElementFactory();
        }
        return instance;
    }
     ...
     *
     * @param name  the parameter name, must not be null or empty
     * @param value the parameter value, an be null if not yet known
     *
     ...
     */
    public Parameter createPiCheckFormatParameter(final String value) throws ParamValidateException {
     ...
     * request.
     *
     * @param url        the input product's URL, must not be null
     * @param fileFormat the file format, can be null if not known
     * @param typeId     the product type identifier, can be null if not known
     *
     ...
     */
    @Override
    public ProductRef createInputProductRef(final File url, final String fileFormat, final String typeId)
            throws RequestElementFactoryException {
     ...
     * request.
     *
     * @param url        the output product's URL, must not be null
     * @param fileFormat the file format, can be null if not known
     * @param typeId     the product type identifier, can be null if not known
     *
     * @throws IllegalArgumentException if url is null
     ...
     */
    @Override
    public ProductRef createOutputProductRef(final File url, final String fileFormat, final String typeId)
            throws RequestElementFactoryException {
        return _defFactory.createOutputProductRef(url, fileFormat, typeId);
    }
