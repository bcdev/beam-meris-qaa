...
@OperatorMetadata(alias = "Meris.waterClarity",
...
*/
    @Parameter(defaultValue = "true", label = "Divide water reflectance by PI(3.14)",
               description = "If selected the source remote reflectance are divided by PI")
...
//
        ...
        */
        ...
        String discriptionEZ=bandnameEZ;
         addBand(configurer, bandnameEZ, 490,
                    discriptionEZ); //y.jiang
...
//        try{
//        targetSamples[WC_INDEXES[0]].set(sourceSamples[0].getFloat());}catch(Exception e){ }
          if (isSampleValid(x, y)) { // Check if it is water
            final float[] rrs = new float[sourceSamples.length-1];  //y. jiang
...
//                   targetSamples[A_INDEXES[i]].set(a);
               ...
               for(int i=0;i< WC_INDEXES.length;i++)
                 ...
                 else throw new NegativeNumberException("Will produce an negative number", z);
